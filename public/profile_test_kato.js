...
const betvalueField =  document.getElementById("kakekin");
document.querySelector("#bet").onclick = 
async(event) => {
  let betvaluedata = betvalueField.value;
  if(isNaN(betvaluedata)) return;
  let betvalue = Number(betvaluedata)//int型に変換
  let targetuid = getParam('targetuid');
